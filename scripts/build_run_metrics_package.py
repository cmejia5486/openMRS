...
def _registered_contract_rows() -> List[Dict[str, Any]]:
    ...
    for pid, contract in PROMPT_CONTRACTS.items():
        row = dict(contract); row["prompt_id"] = pid; row.setdefault("registration_status", "registered"); out.append(row)
    return out
...
def _build_prompt_sections(prompt_calls: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]], str]:
    ...
    for c in prompt_calls:
        ...
        if pid and pid not in registered:
            registered[pid] = {"prompt_id": pid, "prompt_name": c.get("prompt_name", "Unregistered prompt"), "prompt_scope": c.get("prompt_scope", "unregistered"), "prompt_category": "unregistered_candidate", "registration_status": "unregistered", "source_file": c.get("source_file", ""), "source_function": c.get("source_function", "")}
    contracts = []
    control_breakdown = []
    run_results = []
    controls_summary = []
    ...
    for pid in sorted(registered):
        ...
        for idx, clause in enumerate(control_lines, start=1):
            control_name = "registered control clause"
            for field, label in CONTROL_LABELS.items():
                key_words = label.lower().split()
                if any(word in clause.lower() for word in key_words[:2]):
                    control_name = label
                    break
            metric = "prompt_success_rate"
            if "json" in clause.lower(): metric = "json_valid_rate"
            elif "schema" in clause.lower() or "field" in clause.lower(): metric = "schema_valid_rate"
            elif "id" in clause.lower() or "puid" in clause.lower(): metric = "traceability_ok_rate"
            elif "fallback" in clause.lower(): metric = "fallback_rate"
            elif "retry" in clause.lower() or "repair" in clause.lower(): metric = "retry_rate"
...
def _input_evidence_rows(wb: Any, args: argparse.Namespace, telemetry_components: List[Dict[str, Any]], output_xlsx: Path, output_pdf: Path, manifest_path: Path) -> List[Dict[str, Any]]:
    ...
    def add(name: str, group: str, role: str, path_value: str, purpose: str, stability_key: str = "yes") -> None:
        ...
        rows.append({"input_name": name, "evidence_group": group, "evidence_role": role, "artifact_type": typ, "path": str(p) if path_value else "", "file_count": count, "sha256": sha, "availability": "available" if sha else "missing", "purpose": purpose, "used_for_stability_comparability": stability_key})
